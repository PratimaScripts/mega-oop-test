...
class CountryCode extends React.Component {
  ...
  open = () => {
    this.setState({ isOpen: !this.state.isCountryCodeSelectorOpen });
  };

  render() {
    const { isCountryCodeSelectorOpen } = this.props;

    return (
      <div className="countryCodeDropdown">
        <Select
          onChange={e => this.props.updateSelectedCountryCode(e)}
          style={{
            width: "35px"
          }}
          open={isCountryCodeSelectorOpen}
        >
          {ar &&
            ar.map((c, i) => {
              return (
                <Option value={c.dial_code} key={c.dial_code}>
                  <Img
                    style={{ width: "20px" }}
                    src={[
                      `/assets/flags/${c.code.toLowerCase()}.webp`,
